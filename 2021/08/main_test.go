...
func TestReadInput(t *testing.T) {
	...
	require.Equal(t, expectedEntries, input)
}

func TestCountIdentifiableValues(t *testing.T) {
	_, total := countIdentifiableValues(expectedEntries)

	require.Equal(t, 26, total)
}

func TestSumOutputValues(t *testing.T) {
	total, err := sumOutputValues(expectedEntries)
	require.NoError(t, err)
	require.Equal(t, 61229, total)
}
